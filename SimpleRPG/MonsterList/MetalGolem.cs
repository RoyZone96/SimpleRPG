...
public class MetalGolem : IMonster
{
    ...
    public void HeavySmash(ICharacter target)
    {
        if (Mana < 10)
        {
            Console.WriteLine($"{Name} does not have enough Mana to use Heavy Smash!");
            Attack(target);
            return;
        }
        Mana -= 10;
        ...
        {
            Console.WriteLine($"{target.Name} evaded the Heavy Smash!");
            return;
        }
        ...
        Console.WriteLine($"{Name} uses Heavy Smash on {target.Name} for {damage} damage. {target.Name} has {target.Health} health left.");
    }
